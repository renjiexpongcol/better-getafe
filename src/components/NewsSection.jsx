  ...
  return (
    <section className="section home-news-section" id="news">
      <div className="container">
        ...
        {loading ? <p className="home-news-state">Loading the latest updates…</p> : articles.length ? <div className="home-news-grid">{articles.map((article, index) => <article className={index === 0 ? 'home-news-card home-news-card-featured' : 'home-news-card'} key={article.id}><div className="home-news-card-icon">{article.featured_image ? <img src={article.featured_image} alt="" /> : <Newspaper size={22} />}</div><div className="home-news-card-body"><p className="news-category">{article.category?.name || 'Updates'}</p><h3>{article.title}</h3><p>{article.excerpt}</p><Link to={`/news/${article.slug}`}>Read story <ArrowUpRight size={15} /></Link></div></article>)}</div> : <div className="home-news-empty"><Newspaper size={22} /><p>New municipal announcements will appear here.</p><Link to="/news">Visit News &amp; Updates</Link></div>}
      </div>
    </section>
  )
}
