...
export default function Admin() {
 ...
 const load = async () => { try { const [news, cats, files] = await Promise.all([api('/api/news?limit=50', token), api('/api/categories'), api('/api/media', token)]); setArticles(news.items); setCategories(cats); setMedia(files) } catch { setToken(''); localStorage.removeItem('getafe_cms_token') } }
 ...
 const upload = async e => { const file = e.target.files?.[0]; if (!file) return; if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 5 * 1024 * 1024) return setNotice('Choose a JPG, PNG, or WEBP image smaller than 5 MB.'); const reader = new FileReader(); reader.onload = async () => { try { const item = await api('/api/media', token, { method: 'POST', body: JSON.stringify({ name: file.name, dataUrl: reader.result }) }); setMedia(m => [item, ...m]); setForm(f => ({ ...f, featured_image: item.filepath })); setNotice('Image uploaded and selected.') } catch (err) { setNotice(err.message) } }; reader.readAsDataURL(file) }
 if (!user || !token) return <Navigate to="/auth/login" replace />
 if (user.role !== 'admin') return <Navigate to="/" replace />
 const published = articles.filter(a => a.status === 'published').length
 return <main className="cms"><aside className="cms-side"><div><img src="/assets/getafe-seal.png" alt="" /><strong>Getafe CMS</strong></div>{[['dashboard','Dashboard'],['news','News & Updates'],['categories','Categories'],['media','Media Library'],['settings','Settings']].map(([key,label]) => <button className={tab === key ? 'active' : ''} onClick={() => setTab(key)} key={key}>{label}</button>)}<button onClick={logout}>Logout</button><Link to="/news">View public news ↗</Link></aside><section className="cms-main">{notice && <div className="cms-notice">{notice}</div>}{tab === 'dashboard' && <><div className="cms-title"><div><p className="news-category">Overview</p><h1>Dashboard</h1></div><button onClick={() => { setForm(blank); setEditing(null); setTab('editor') }}>+ Add News</button></div><div className="cms-stats"><div><span>Published news</span><strong>{published}</strong></div><div><span>Drafts</span><strong>{articles.length - published}</strong></div><div><span>Categories</span><strong>{categories.length}</strong></div></div><h2>Recent articles</h2><ArticleTable articles={articles.slice(0, 5)} edit={a => { setForm({ ...a, published_at: a.published_at?.slice(0, 16) || '' }); setEditing(a.id); setTab('editor') }} remove={removeArticle} /></>}{tab === 'news' && <><div className="cms-title"><h1>News & Updates</h1><button onClick={() => { setForm(blank); setEditing(null); setTab('editor') }}>+ Add News</button></div><ArticleTable articles={articles} edit={a => { setForm({ ...a, published_at: a.published_at?.slice(0, 16) || '' }); setEditing(a.id); setTab('editor') }} remove={removeArticle} /></>}{tab === 'editor' && <form className="cms-editor" onSubmit={saveArticle}><div className="cms-title"><h1>{editing ? 'Edit article' : 'Add news article'}</h1><button>{form.status === 'published' ? 'Publish changes' : 'Save draft'}</button></div><label>Title<input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value, slug: form.slug || e.target.value.toLowerCase().replace(/[^a-z0-9]+/g,'-') })} /></label><label>URL slug<input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} /></label><label>Short excerpt<textarea required value={form.excerpt} onChange={e => setForm({ ...form, excerpt: e.target.value })} /></label><label>Full content<textarea className="content-box" required value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} /><small>HTML is supported (for example, &lt;p&gt;paragraph&lt;/p&gt;).</small></label><div className="cms-row"><label>Category<select value={form.category_id} onChange={e => setForm({ ...form, category_id: e.target.value })}><option value="">Uncategorised</option>{categories.map(c => <option value={c.id} key={c.id}>{c.name}</option>)}</select></label><label>Status<select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}><option value="draft">Draft</option><option value="published">Published</option></select></label><label>Publication date<input type="datetime-local" value={form.published_at} onChange={e => setForm({ ...form, published_at: e.target.value })} /></label></div><label>Featured image<input type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} /></label>{form.featured_image && <img className="cms-preview" src={form.featured_image} alt="Selected" />}</form>}{tab === 'categories' && <Categories token={token} categories={categories} reload={load} />}{tab === 'media' && <><div className="cms-title"><h1>Media Library</h1><label className="upload-btn">Upload image<input type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} /></label></div><div className="media-grid">{media.map(m => <div key={m.id}><img src={m.filepath} alt={m.filename} /><small>{m.filename}</small><button onClick={async () => { if (confirm('Delete this image?')) { try { await api(`/api/media/${m.id}`, token, { method: 'DELETE' }); load() } catch (e) { setNotice(e.message) } } }}>Delete</button></div>)}</div></>}{tab === 'settings' && <Settings token={token} settings={settings} setSettings={setSettings} onSaved={setNotice} />}</section></main>
}
