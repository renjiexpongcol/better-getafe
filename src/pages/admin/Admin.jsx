...
const putFile = (url, file, onProgress) => {
 ...
 const promise = new Promise((resolve, reject) => {
  ...
  request.onload = () => request.status >= 200 && request.status < 300 ? resolve() : reject(new Error('Direct upload to GCS failed'))
...
export default function Admin() {
 ...
 const upload = async e => {
  ...
  if (!file) return;
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 5 * 1024 * 1024) return setNotice('Choose a JPG, PNG, or WEBP image smaller than 5 MB.');
  try {
    ...
    setNotice('Uploading to Google Cloud Storage...');
    uploadRequest.current = putFile(urlData.uploadUrl, file, progress => setNotice(`Uploading to Google Cloud Storage... ${progress}%`));
    ...
    if (!completeRes.ok) throw new Error(body?.error || 'Upload completion failed');

    setMedia(m => [body, ...m]);
    setForm(f => ({ ...f, featured_image: body.storage_path, featured_image_preview: body.preview_url || body.storage_path }));
