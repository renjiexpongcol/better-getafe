...
const actions = [
  ['Request Document', FilePlus2, '/app?sysparm_object_id=services&category=certificates'], ['Apply for Permit', BriefcaseBusiness, '/app?sysparm_object_id=services&category=business'],
  ['Book Appointment', CalendarDays, '/app?sysparm_object_id=appointments&book=1'],
  ['Pay Fees', CreditCard, '/app?sysparm_object_id=payments'], ['Report Concern', MessageSquare, '/app?sysparm_object_id=requests&sysparm_view=new&service=concern'],
]
const quickFields = {
...
function DashboardContent() {
  ...
  const stats = [
    ['Active Applications', FileText, active.length, attention.length ? `${attention.length} requires attention` : active.length ? 'Track your requests' : 'No active requests', '/app?sysparm_object_id=requests'],
    ['Available Documents', FolderOpen, available.length, available.length ? 'View your documents' : 'No documents available', '/app?sysparm_object_id=documents'],
    ['Upcoming Appointments', CalendarDays, appointments.length, appointments.length ? dateLabel(appointments[0].appointment_at, { year: undefined, hour: 'numeric', minute: '2-digit' }) : 'No appointments scheduled', '/app?sysparm_object_id=appointments'],
    ['Outstanding Payments', CreditCard, money(due.reduce((sum, item) => sum + Number(item.amount), 0)), due.length ? `${due.length} ${due.length === 1 ? 'payment' : 'payments'} due` : 'No outstanding payments', '/app?sysparm_object_id=payments'],
  ]
  ...
  return <>
    ...
    {!pending && (attention.length > 0 || due.length > 0) && <section className="portal-attention"><h2><CircleAlert size={20}/>Requires your attention<span>{attention.length + due.length}</span></h2>{attention.map(item => <article key={item.id}><div><h3>{item.service_name}</h3><p>{item.latest_note || 'Additional information is required to continue processing.'}</p></div><Link to={`/app?sysparm_object_id=requests&sysparm_record_id=${encodeURIComponent(item.id)}`}>Continue application<ArrowRight size={15}/></Link></article>)}{due.map(item => <article key={item.id}><div><h3>Payment required · {item.service_name || 'Municipal service'}</h3><p>{money(item.amount)} outstanding</p></div><Link to={`/app?sysparm_object_id=payments&sysparm_record_id=${encodeURIComponent(item.id)}`}>View payment<ArrowRight size={15}/></Link></article>)}</section>}
    <section className="portal-quick"><h2>Quick actions</h2><div>{actions.map(([label, Icon]) => <button type="button" key={label} onClick={() => label === 'Book Appointment' ? navigate('/app?sysparm_object_id=appointments&book=1') : setQuickAction({ label, Icon })}><Icon size={20}/><span>{label}</span></button>)}</div></section>
    <ServiceCategories/>
    <div className="citizen-lower">
      <Panel title="My applications" href="/app?sysparm_object_id=requests"><DataState><ApplicationRows items={applications.slice(0, 4)}/></DataState></Panel>
      <Panel title="My documents" href="/app?sysparm_object_id=documents"><DataState><DocumentRows items={documents.slice(0, 3)}/></DataState></Panel>
      <Panel title="Upcoming appointment" href="/app?sysparm_object_id=appointments"><DataState><AppointmentRows items={appointments.slice(0, 1)}/></DataState></Panel>
      <Panel title="Recent activity" href="/app?sysparm_object_id=notifications"><DataState>{activity.length ? <ol className="portal-activity">{activity.slice(0, 4).map(item => <li key={item.id}><span><Bell size={14}/></span><div><Link to={notificationLink(item)}>{item.title}</Link><p>{item.message}</p><time>{dateLabel(item.created_at, { hour: 'numeric', minute: '2-digit' })}</time></div></li>)}</ol> : <Empty icon={Bell} title="No activity yet" description="Your application updates, payments and document activity will appear here."/>}</DataState></Panel>
    </div>
    ...
    {(params.get('profile') === '1' || params.get('sysparm_object_id') === 'profile') && <div className="profile-modal-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && setParams({ sysparm_object_id: 'dashboard' })}><section className="profile-modal-card"><ProfileForm onClose={() => setParams({ sysparm_object_id: 'dashboard' })}/></section></div>}
  </>
}
