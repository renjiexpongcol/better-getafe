...
import { id, now } from '../repositories/localDb.js'
...
export function installAccessManagementRoutes(app, admin) {
  ...
  app.get('/api/admin/access', async (req, res) => {
    try {
      ...
      const groups = db.prepare(`SELECT g.*, COUNT(DISTINCT ug.user_id) AS member_count, COUNT(DISTINCT gp.permission_id) AS permission_count
      ...
      const permissions = db.prepare('SELECT * FROM auth_permissions ORDER BY category,label').all()
      const policies = db.prepare('SELECT * FROM auth_policies ORDER BY name').all().map(item => ({ ...item, condition: parseJson(item.condition_json) }))
      const memberships = db.prepare('SELECT ug.user_id,g.id,g.name FROM auth_user_groups ug JOIN auth_groups g ON g.id=ug.group_id WHERE g.archived_at IS NULL ORDER BY g.name').all()
      ...
      const activity = db.prepare('SELECT * FROM auth_audit_logs ORDER BY created_at DESC LIMIT 100').all()
  ...
  app.get('/api/admin/access/groups/:id', async (req, res) => {
    try {
      const db = await ensureAuthorizationData(), group = db.prepare('SELECT * FROM auth_groups WHERE id=?').get(req.params.id)
      if (!group) return res.status(404).json({ error: 'Group not found.' })
      const members = db.prepare('SELECT user_id,created_at,created_by FROM auth_user_groups WHERE group_id=? ORDER BY created_at').all(group.id)
      const permissions = db.prepare('SELECT p.* FROM auth_permissions p JOIN auth_group_permissions gp ON gp.permission_id=p.id WHERE gp.group_id=? ORDER BY p.category,p.label').all(group.id)
      const policies = db.prepare('SELECT p.* FROM auth_policies p JOIN auth_group_policies gp ON gp.policy_id=p.id WHERE gp.group_id=? ORDER BY p.name').all(group.id).map(item => ({ ...item, condition: parseJson(item.condition_json) }))
      const activity = db.prepare("SELECT * FROM auth_audit_logs WHERE target_id=? OR (target_type='membership' AND new_value LIKE ?) ORDER BY created_at DESC LIMIT 100").all(group.id, `%${group.id}%`)
  ...
  app.post('/api/admin/access/groups', async (req, res) => {
    try {
      ...
      if (db.prepare('SELECT 1 FROM auth_groups WHERE lower(name)=lower(?)').get(name)) return res.status(409).json({ error: 'A group with this name already exists.' })
      const permissionIds = ids(req.body?.permission_ids), valid = new Set(db.prepare('SELECT id FROM auth_permissions').all().map(item => item.id)), actorAccess = await effectiveAccessFor(req.admin), actorPermissions = new Set(actorAccess.permissions.filter(permission => permission.allowed).map(permission => permission.id))
      ...
      db.exec('BEGIN IMMEDIATE'); try {
        db.prepare('INSERT INTO auth_groups (id,name,description,enabled,system_group,created_at,updated_at) VALUES (?,?,?,?,?,?,?)').run(groupId, name, description, 1, 0, timestamp, timestamp)
        const link = db.prepare('INSERT INTO auth_group_permissions (group_id,permission_id,created_at,created_by) VALUES (?,?,?,?)')
        for (const permissionId of permissionIds) link.run(groupId, permissionId, timestamp, req.admin.id)
        audit(db, req.admin.id, 'GROUP_CREATED', 'group', groupId, null, { name, description, permission_ids: permissionIds }, correlationId)
        db.exec('COMMIT')
  ...
  app.patch('/api/admin/access/groups/:id', async (req, res) => {
    try {
      ...
      db.prepare('UPDATE auth_groups SET name=?,description=?,enabled=?,updated_at=? WHERE id=?').run(name, description, enabled, now(), group.id)
      audit(db, req.admin.id, enabled ? 'GROUP_UPDATED' : 'GROUP_DISABLED', 'group', group.id, group, { name, description, enabled: Boolean(enabled) })
      res.json({ saved: true })
  ...
  app.delete('/api/admin/access/groups/:id', async (req, res) => {
    try {
      ...
      const members = db.prepare('SELECT COUNT(*) AS count FROM auth_user_groups WHERE group_id=?').get(group.id).count
      if (members) return res.status(409).json({ error: 'Remove all group members before archiving this group.' })
      const archivedAt = now()
      db.prepare('UPDATE auth_groups SET enabled=0,archived_at=?,updated_at=? WHERE id=?').run(archivedAt, archivedAt, group.id)
      audit(db, req.admin.id, 'GROUP_ARCHIVED', 'group', group.id, group, { ...group, enabled: false, archived_at: archivedAt })
  ...
  app.put('/api/admin/access/groups/:id/permissions', async (req, res) => {
    try {
      ...
      if (permissionIds.some(permission => !actorPermissions.has(permission))) return res.status(403).json({ error: 'You cannot grant a permission that you do not hold.' })
      const valid = new Set(db.prepare('SELECT id FROM auth_permissions').all().map(item => item.id))
      if (permissionIds.some(permission => !valid.has(permission))) return res.status(422).json({ error: 'One or more permissions are invalid.' })
      const previous = db.prepare('SELECT permission_id FROM auth_group_permissions WHERE group_id=?').all(group.id).map(item => item.permission_id)
      db.exec('BEGIN IMMEDIATE'); try {
        db.prepare('DELETE FROM auth_group_permissions WHERE group_id=?').run(group.id)
        const insert = db.prepare('INSERT INTO auth_group_permissions (group_id,permission_id,created_at,created_by) VALUES (?,?,?,?)')
        for (const permission of permissionIds) insert.run(group.id, permission, now(), req.admin.id)
        for (const permission of permissionIds.filter(item => !previous.includes(item))) audit(db, req.admin.id, 'PERMISSION_ADDED_TO_GROUP', 'group', group.id, null, { permission_id: permission })
        for (const permission of previous.filter(item => !permissionIds.includes(item))) audit(db, req.admin.id, 'PERMISSION_REMOVED_FROM_GROUP', 'group', group.id, { permission_id: permission }, null)
        db.exec('COMMIT')
      } catch (error) { db.exec('ROLLBACK'); throw error }
      res.json({ saved: true })
  ...
  app.put('/api/admin/access/groups/:id/members', async (req, res) => {
    try {
      ...
      if (userIds.includes(req.admin.id) !== Boolean(db.prepare('SELECT 1 FROM auth_user_groups WHERE user_id=? AND group_id=?').get(req.admin.id, group.id))) return res.status(403).json({ error: 'You cannot change your own group membership.' })
      const previous = db.prepare('SELECT user_id FROM auth_user_groups WHERE group_id=?').all(group.id).map(item => item.user_id)
      ...
      db.exec('BEGIN IMMEDIATE'); try {
        db.prepare('DELETE FROM auth_user_groups WHERE group_id=?').run(group.id)
        const insert = db.prepare('INSERT INTO auth_user_groups (user_id,group_id,created_at,created_by) VALUES (?,?,?,?)')
        for (const userId of userIds) insert.run(userId, group.id, now(), req.admin.id)
        for (const userId of userIds.filter(item => !previous.includes(item))) audit(db, req.admin.id, 'USER_ADDED_TO_GROUP', 'membership', userId, null, { user_id: userId, group_id: group.id })
        for (const userId of previous.filter(item => !userIds.includes(item))) audit(db, req.admin.id, 'USER_REMOVED_FROM_GROUP', 'membership', userId, { user_id: userId, group_id: group.id }, null)
        db.exec('COMMIT')
      } catch (error) { db.exec('ROLLBACK'); throw error }
      res.json({ saved: true })
  ...
  app.put('/api/admin/access/users/:id/groups', async (req, res) => {
    try {
      ...
      const groupIds = ids(req.body?.group_ids), groups = db.prepare('SELECT * FROM auth_groups WHERE archived_at IS NULL').all(), valid = new Set(groups.map(group => group.id))
      if (groupIds.some(groupId => !valid.has(groupId))) return res.status(422).json({ error: 'One or more groups are invalid.' })
      const previous = db.prepare('SELECT group_id FROM auth_user_groups WHERE user_id=?').all(target.id).map(item => item.group_id)
      ...
      for (const groupId of newlyAssigned) {
        const required = db.prepare('SELECT permission_id FROM auth_group_permissions WHERE group_id=?').all(groupId).map(item => item.permission_id)
      ...
      db.exec('BEGIN IMMEDIATE'); try {
        db.prepare('DELETE FROM auth_user_groups WHERE user_id=?').run(target.id)
        const insert = db.prepare('INSERT INTO auth_user_groups (user_id,group_id,created_at,created_by) VALUES (?,?,?,?)')
        for (const groupId of groupIds) insert.run(target.id, groupId, now(), req.admin.id)
        for (const groupId of groupIds.filter(item => !previous.includes(item))) audit(db, req.admin.id, 'USER_ADDED_TO_GROUP', 'membership', target.id, null, { user_id: target.id, group_id: groupId })
        for (const groupId of previous.filter(item => !groupIds.includes(item))) audit(db, req.admin.id, 'USER_REMOVED_FROM_GROUP', 'membership', target.id, { user_id: target.id, group_id: groupId }, null)
        db.exec('COMMIT')
      } catch (error) { db.exec('ROLLBACK'); throw error }
      res.json({ saved: true })
  ...
  app.post('/api/admin/access/policies', async (req, res) => {
    try {
      ...
      if (permissionId && !db.prepare('SELECT 1 FROM auth_permissions WHERE id=?').get(permissionId)) return res.status(422).json({ error: 'Choose a valid permission.' })
      ...
      const policyId = id(), timestamp = now(); db.prepare('INSERT INTO auth_policies (id,name,description,effect,permission_id,condition_json,enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)').run(policyId, name, description, effect, permissionId, JSON.stringify(condition), 1, timestamp, timestamp); audit(db, req.admin.id, 'POLICY_CREATED', 'policy', policyId, null, { name, description, effect, permissionId, condition }); res.status(201).json({ id: policyId })
  ...
  app.patch('/api/admin/access/policies/:id', async (req, res) => {
    try {
      await requireAccess(req.admin, 'policies.manage')
      const db = await ensureAuthorizationData(), policy = db.prepare('SELECT * FROM auth_policies WHERE id=?').get(req.params.id)
      ...
      const updatedAt = now()
      db.prepare('UPDATE auth_policies SET name=?,description=?,effect=?,permission_id=?,condition_json=?,enabled=?,updated_at=? WHERE id=?').run(name, description, effect, permissionId, JSON.stringify(condition), enabled, updatedAt, policy.id)
      audit(db, req.admin.id, enabled ? 'POLICY_UPDATED' : 'POLICY_DISABLED', 'policy', policy.id, { ...policy, condition: parseJson(policy.condition_json) }, { name, description, effect, permissionId, condition, enabled: Boolean(enabled) })
      res.json({ saved: true })
  ...
  app.put('/api/admin/access/groups/:id/policies', async (req, res) => {
    try {
      await requireAccess(req.admin, 'policies.manage')
      const db = await ensureAuthorizationData(), policyIds = ids(req.body?.policy_ids), valid = new Set(db.prepare('SELECT id FROM auth_policies').all().map(item => item.id))
      if (!db.prepare('SELECT 1 FROM auth_groups WHERE id=? AND archived_at IS NULL').get(req.params.id)) return res.status(404).json({ error: 'Group not found.' })
      if (policyIds.some(policy => !valid.has(policy))) return res.status(422).json({ error: 'One or more policies are invalid.' })
      const actorAccess = await effectiveAccessFor(req.admin), actorPermissions = new Set(actorAccess.permissions.filter(permission => permission.allowed).map(permission => permission.id))
      const policyPermissions = db.prepare(`SELECT DISTINCT permission_id FROM auth_policies WHERE id IN (${policyIds.map(() => '?').join(',') || "''"}) AND permission_id IS NOT NULL`).all(...policyIds).map(item => item.permission_id)
      ...
      const previous = db.prepare('SELECT policy_id FROM auth_group_policies WHERE group_id=?').all(req.params.id).map(item => item.policy_id)
      db.exec('BEGIN IMMEDIATE'); try { db.prepare('DELETE FROM auth_group_policies WHERE group_id=?').run(req.params.id); const insert = db.prepare('INSERT INTO auth_group_policies (group_id,policy_id,created_at,created_by) VALUES (?,?,?,?)'); for (const policyId of policyIds) insert.run(req.params.id, policyId, now(), req.admin.id); audit(db, req.admin.id, 'GROUP_POLICIES_UPDATED', 'group', req.params.id, previous, policyIds); db.exec('COMMIT') } catch (error) { db.exec('ROLLBACK'); throw error }
      res.json({ saved: true })
